let mostGiftedGetter = async () => {
  ...
  let filterGames = res.filter(
    (gifted) => Number(gifted.id) > 30 && Number(gifted.id) <= 40
  );
  let data = filterGames.map((elem) => {
...
export default mostGiftedGetter;
